    ...
    if (status === 200 && code === 0 && list.length) {
      return {
        list: list.map(item => {
          // 数据映射
          return {
            img: item.imgs.length ? item.imgs[0] : "//s0.meituan.net/bs/fe-web-meituan/e5eeaef/img/logo.png", // 可以提供默认图片
            name: item.name,
            count: 1,
            total: item.total,
            status: item.status,
        ...
        cur: list.map(item => {
          return {
            img: "//s0.meituan.net/bs/fe-web-meituan/e5eeaef/img/logo.png",
            name: item.name,
            count: 1,
            total: item.total,
            status: item.status,
